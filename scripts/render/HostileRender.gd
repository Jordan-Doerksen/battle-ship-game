...
static func draw_enemies(r: FieldRenderer) -> void:
	...
	for e in r._world.enemies:
		...
		r.draw_set_transform(draw_pos, e.heading + rock, Vector2.ONE)
		if e.type_id == "swarmer":
			...
			r.draw_polyline(bc, Color(0.851, 0.310, 0.169, 0.8), 1.2, true)
			...
			r.draw_polyline(wc, Color(0.851, 0.310, 0.169, 0.9), 1.2, true)
...
static func draw_sub(r: FieldRenderer, e: Enemy, now: float) -> void:
	if Sonar.detected(r._world, e):
		...
		draw_ellipse_outline(r, 7.0, 20.0, Color(FieldRenderer.RED.r, FieldRenderer.RED.g, FieldRenderer.RED.b, 0.55), 1.2)
		...
		r.draw_arc(Vector2.ZERO, 24.0 + sin(now * 0.004) * 2.0, 0.0, TAU, 32,
			Color(FieldRenderer.FOAM.r, FieldRenderer.FOAM.g, FieldRenderer.FOAM.b, 0.25), 1.0, true)   # foam ring over the contact
		...
		r.draw_arc(Vector2.ZERO, 14.0 + wob, 0.0, TAU, 28,
			Color(FieldRenderer.FOAM.r, FieldRenderer.FOAM.g, FieldRenderer.FOAM.b, 0.07), 1.5, true)
		r.draw_arc(Vector2.ZERO, 24.0 - wob, 0.0, TAU, 32,
			Color(FieldRenderer.FOAM.r, FieldRenderer.FOAM.g, FieldRenderer.FOAM.b, 0.045), 1.5, true)
	r.draw_set_transform(Vector2.ZERO, 0.0, Vector2.ONE)
